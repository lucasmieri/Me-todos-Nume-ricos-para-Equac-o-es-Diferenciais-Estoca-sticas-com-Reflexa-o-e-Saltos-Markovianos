     ...
     //   aux = alfa_1*(X_step*sigma[A])*(X_step*sigma[A])-alfa_2*(X*sigma[A])*(X*sigma[A]);
       // aux = (aux>0)? aux:0;
        ...
        //X = X_step + (alfa_1*mu[A]*X_step - alfa_2*mu[A]*X)*(1-theta)*Dt
          ...
          X= X +mu[A]*X_step*Dt+ sigma[A]*X*norms1[j]*sqrt(Dt) ;
...
// [[Rcpp::export]]
NumericVector TrapFuncRcpp(unsigned int type, double Dt, unsigned int L, unsigned int M)
{
    //Global env data
